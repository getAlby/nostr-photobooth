...
const SearchBox = ({ addPerson, text, setText, badgeName, setBadgeName }) => {
  ...
  const search = useCallback(async (query) => {
    ...
    const res = resp.data.people.slice(0, 3);
      ...
      <form className="relative">
        <div className="relative">
          <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
            {loading ? (
              <svg
                className="w-6 h-6 text-neutral-600 animate-spin fill-primary"
                viewBox="0 0 100 100"
                fill="none"
              ...
              <Search
                className={`${isFocused ? 'text-primary' : 'text-neutral-600'}`}
              />
            )}
          </div>
          <input
        ...
        {modal && (
          <div
            ref={searchRef}
            className="absolute w-full bg-surface-06dp border border-neutral-700 rounded-md shadow-md mt-4 py-1"
          >
            {!people.length && (
            ...
            {people.map((person, index) => (
              <div
                key={`person-${index}`}
                className="hover:bg-surface-24dp cursor-pointer flex justify-between items-center text-white"
                onClick={() => {
                  addPerson(person);
                  setModal(false);
                }}
